...
download_repo() {
    ...
    git clone https://github.com/gchengyu/PagerMaid-Modify.git /var/lib/pagermaid01 >>/dev/null 2>&1
...
configure() {
    ...
    [yY][eE][sS] | [yY])
        printf " 请输入您的日志记录群组 / 频道的 ChatID （如果要发送给 原 PagerMaid 作者 ，请按 Enter）："
        read -r log_chatid <&1
